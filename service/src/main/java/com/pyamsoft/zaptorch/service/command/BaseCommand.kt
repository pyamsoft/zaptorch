...
protected constructor(private val preferences: TorchPreferences) : Command {
  ...
  private suspend fun ensureScope(): CoroutineScope =
      mutex.withLock { commandScope ?: newScope().also { commandScope = it } }
  ...
  internal suspend fun handleTorchOnFirstPress() = coroutineScope {
    mutex.withLock { commandReady = true }
  ...
  final override suspend fun handle(keyCode: Int, handler: Command.Handler): Boolean {
    ...
    return if (!isKeyCodeHandled(keyCode, isReady)) false
    else {
